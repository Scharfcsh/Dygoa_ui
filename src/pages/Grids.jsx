...
const EcoGridDashboard = () => {
  ...
  const getProduction = async () => {
    try {
      ...
      const response = await axios.get(
        `https://shot-attacks-indicators-valve.trycloudflare.com/api/grids/production`
      );
            ...
            <div className="p-6 border-b border-gray-200 rounded-md">
              ...
              <div className="flex items-center justify-between">
                <p className="text-gray-600 text-lg">
                  <span className="font-bold">Current Production:</span>{" "}
                  {currentEnergy} W
                </p>
                <div className="flex items-center space-x-2">
                  <span className="font-bold text-gray-600">Total demand:</span>
                  <span className="text-2xl font-bold text-green-600">
                    {Math.round(gridDetails.totalWattage)} W
                  </span>
                </div>
              </div>
              <div className="mt-4 bg-gray-200 rounded-full h-4 overflow-hidden">
                <div
                  className={
                    toggleDemandColor(currentEnergy, gridDetails.totalWattage) +
                    " h-full transition-all duration-500 ease-in-out "
                  }
                  style={{
                    width: `${
                      (gridDetails.totalWattage / currentEnergy) * 100
                    }%`,
                  }}
                ></div>
              </div>
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <div className={`flex flex-col`}>
                              <span className="`flex flex-col font-bold ${building.status ? 'text-green-500' : 'text-red-500'}`">
                                {building.status}
                              </span>
                              <button
                                onClick={() =>
